...
static irqreturn_t ft5x06_handler(int irq, void *dev_id)
{
	...
	for (i = 0; i < MAX_SUPPORT_POINTS; i++) {
		...
		x = FT_MAX_X - (((buf[0] << 8) | buf[1]) & 0x0fff);
		y = FT_MAX_Y - (((buf[2] << 8) | buf[3]) & 0x0fff);
...
static int ft5x06_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* 1，获取设备树中的中断和复位引脚 */
	ft5x06.irq_pin = of_get_named_gpio(client->dev.of_node, "ft5x26,irq-gpio", 0);
	ft5x06.reset_pin = of_get_named_gpio(client->dev.of_node, "ft5x26,rst-gpio", 0);
	
	printk(KERN_INFO "irq_pin = %d ; rst_pin = %d;\n", ft5x06.irq_pin, ft5x06.reset_pin);	
	...
	input_set_abs_params(ft5x06.input, ABS_X, 0, FT_MAX_X, 0, 0);
	input_set_abs_params(ft5x06.input, ABS_Y, 0, FT_MAX_Y, 0, 0);
	input_set_abs_params(ft5x06.input, ABS_MT_POSITION_X,0, FT_MAX_X, 0, 0);
	input_set_abs_params(ft5x06.input, ABS_MT_POSITION_Y,0, FT_MAX_Y, 0, 0);	     
